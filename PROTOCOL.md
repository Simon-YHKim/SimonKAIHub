...
## 26. 자연스러움·직관성·자산일관성 — 페르소나 시뮬 사용성 방법 (CRITICAL, 2026-06-06 Simon)
> Simon 지시: **"앱을 이용하는 동안 딱딱하지 않고 자연스럽고 부드럽게. 효과·UI/UX·애니·테마·디자인 무엇이든. 자산 활용 일관성 점검(섞이거나 이름 다른 것 없게). 불필요 가림·비직관·한 화면 정보과잉(뭘 눌러야 할지 모름) 제거. 영유아~90대 각 10명 다양한 직업군 가상 페르소나로 시뮬레이션. 타 AI에도 이 방법 전파."**

- **26.1 4대 품질 축** (모든 화면/리뷰에 적용):
  1. **자연스러움(smoothness)**: 전환·등장·press 피드백이 부드러운가(즉각 cut 금지). 이징은 부드러운 ease — **bounce/elastic 금지**, tinted-neutral·따뜻함. 모션 perf는 AG 교차검증.
  2. **직관성(intuitive)**: 다음 행동이 추측 없이 보이나. 1차 행동(primary action)이 화면당 하나로 명확한가.
  3. **정보위계(no-overload)**: 한 화면에 내용 과다로 '뭘 눌러야 할지 모름' 금지. 점진적 공개(progressive disclosure)·우선순위.
  4. **자산 일관성(asset)**: 일러스트/아이콘/마스코트 스타일·네이밍 통일. 버전접미사(v1/v3/final/candidate) 혼재·미사용·중복·깨진 경로 제거.
- **26.2 페르소나 시뮬레이션**: 평가 시 **연령 10밴드(영유아·접근성 하한 → 90대) × 직업/상황 다양 페르소나(밴드당 ~10)**를 가정해 첫 실행+핵심 루프를 걷고 마찰을 찾는다. 특히 고령/유아=글자크기·대비·터치타깃·읽기수준 하한, 저 tech-literacy. **상상만 말고 실제 화면 코드 근거**.
- **26.3 오케스트레이션**: 백로그가 클 때 **병렬 페르소나-UX 워크플로**(밴드별 fan-out + 자산 + 모션 감사 → JS 종합)로 발견 → §25 큐의 **E. UX 자연스러움·자산일관성** 버킷에 적재 → 강점 라우팅 소진(stiffness/모션 perf=AG, 카피/정보위계/시각/자산=Codex, 로직/상태/라우팅=Claude, 법무/실설정=Simon).
- **26.4 전파**: 이 방법은 4-AI 공통. 각 AI는 자기 lane에서 페르소나 시뮬 관점을 상시 적용(Codex=정보위계·자산·시각, AG=네이티브 자연스러움·perf·접근성, Grok=페르소나 현실성·세대별 신호).
