...
- **Eval coverage**: `evals/cases.json` for every skill (test coverage 0 → 100%) — 8 eval sets added across the 8 skills.
- **Runtime entry command** under `commands/`: `/skaihub` (the documented slash entry point, previously an empty dir).
...
### Changed
- Rewrote skill descriptions to lead with "Use when …" and state output (routing quality) — 6 descriptions rewritten.

### Fixed
- Migrated stale-schema `evals/cases.json` to the canonical object schema.

[0.2.0]: https://github.com/Simon-YHKim/SimonKAIHub/releases/tag/v0.2.0
