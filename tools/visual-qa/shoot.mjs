...
const ENGINE = arg('engine', 'chromium'); // chromium | webkit | both  (webkit = the real iOS Safari engine + iPhone viewport)
// Each engine is a (browser, context-options) pair. iPhone = WebKit + a real iPhone device descriptor,
// so this captures how 2nd-B WEB renders in iOS Safari. (Native iOS app testing still needs an EAS
// build on Appetize / a real device -- Windows cannot run the iOS Simulator.)
function enginesFor(sel) {
  const list = [];
  if (sel === 'chromium' || sel === 'both') list.push({ key: 'android-chrome', type: chromium, ctx: { viewport: VIEWPORT, deviceScaleFactor: 2 } });
  if (sel === 'webkit' || sel === 'both') list.push({ key: 'iphone-safari', type: webkit, ctx: { ...devices['iPhone 15'] } });
  return list;
}
...
async function main() {
  ...
  const engines = enginesFor(ENGINE);
  const results = [];
  for (const eng of engines) {
    const browser = await eng.type.launch();
    const ctx = await browser.newContext(eng.ctx);
    const page = await ctx.newPage();
    for (const s of screens) {
      const url = BASE + s.path;
      const row = { engine: eng.key, name: s.name, url, verdict: 'OK', meanLum: 0, darkFrac: 0, textLen: 0, note: '' };
      try {
        ...
        const file = join(OUT, eng.key + '-' + s.name + '.png');
      ...
      results.push(row);
      console.log(`  ${eng.key.padEnd(14)} ${row.verdict.padEnd(7)} ${row.name.padEnd(10)} lum=${row.meanLum} dark=${row.darkFrac} text=${row.textLen}  ${row.note}`);
    }
    await browser.close();
  }
  writeFileSync(join(OUT, 'verdict.json'), JSON.stringify({ base: BASE, engine: ENGINE, results }, null, 2));
